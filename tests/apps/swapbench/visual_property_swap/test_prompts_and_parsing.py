...
def test_retrieve_correct_visualswap_pairs_keeps_only_unanimous(tmp_path: Path) -> None:
    ...
    # Field layout (17 fields):
    # idx | bin | rule | w1 | g1 | ig1 | w2 | g2 | ig2 |
    # a1 | a11 | a2 | a22 |   (the four expected answers)
    # ag1 | ag11 | ag2 | ag22  (the four LLM-returned answers)
    rows = [
        "0|3|VISUAL|apple|s1|0|car|s2|0|A|B|A|B|[A]|[B]|[A]|[B]",  # all match -> kept
        "1|3|VISUAL|cat|s1|0|dog|s2|0|A|B|A|B|[A]|[A]|[A]|[B]",  # one mismatch -> dropped
        "2|3|VISUAL|fish|s1|0|fish|s2|0|A|B|A|B|[A]|[B]|[A]|[B]",  # w1==w2 -> dropped
    ]
    src.write_text("\n".join(rows) + "\n")
