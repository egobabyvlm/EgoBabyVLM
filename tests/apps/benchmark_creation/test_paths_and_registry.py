...
def test_resource_paths_resolve_to_existing_files() -> None:
    """The bundled manifest files actually exist on disk."""
    for getter in (paths.get_coco_captions, paths.get_howto100m_manifest, paths.get_babyview_manifest):
        p = getter()
        assert p.is_file(), f"{getter.__name__} → {p} does not exist"
