...
TASK_FILE_PATTERNS = {
    "wordswap": "wordswap_sentence_pairs_filtering_prompts_final_output.txt",
    "visual": "visual_sentence_filtering_prompts_final_output.txt",
    "agrswap": "agrswap_sentence_pairs",
    "inflswap": "inflswap_sentence_pairs",
}
...
class LTSwapEvalConfig(EvalConfig):
    ...
    #: Maps task_type to pair file path.
    pair_files: dict[str, str] = field(default_factory=dict)
...
class LTSwapEvalModule(EvalModule):
    ...
    def _evaluate_task(self, task_type: str, pair_file: str, model: Any) -> dict[str, Any]:
        """Evaluate a single LT-Swap task."""
        logger.info("Evaluating task: %s from %s", task_type, pair_file)

        with Path(pair_file).open() as f:
            pairs = f.readlines()
    ...
    def _get_pair_files(self) -> dict[str, str]:
        """Get pair files for each task type."""
        pair_files = dict(self.config.pair_files) if self.config.pair_files else {}

        if self.config.data_dir:
            data_dir = to_path(self.config.data_dir)
            for task_type in self.config.task_types:
                if task_type not in pair_files:
                    file_pattern = TASK_FILE_PATTERNS.get(task_type, f"{task_type}_sentence_pairs")
    ...
    def run(self, iteration_value: Any = None, iteration_index: int = 0) -> dict[str, Any]:
        ...
        for task_type in self.config.task_types:
            ...
            if not Path(pair_file).exists():
                logger.error("Pair file not found: %s", pair_file)
                continue
