#!/usr/bin/env python
"""Refresh the vendored subset of facebookresearch/perception_models.

Copies the closure of files transitively reachable from ``apps/plm/{tokenizer,
transformer}.py`` into this directory, rewriting all ``from core.X`` and
``from apps.X`` imports to point at the vendored namespace.
...
VENDOR_PREFIX = "apps.alignment_scoring.third_party.perception_models"
...
def _rewrite_imports(text: str) -> str:
    """Rewrite ``from core.X``, ``from apps.X``, ``from core import X``, etc. to vendored namespace."""
    # `from <pkg>.X import Y` and `from <pkg> import Y`.
    text = re.sub(
        r"^(from\s+)(core|apps)((?:\.[\w.]+)?\s+import)",
        rf"\1{VENDOR_PREFIX}.\2\3",
        text,
        flags=re.MULTILINE,
    )
    return re.sub(
