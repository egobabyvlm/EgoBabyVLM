...
#: Minimum field count of a stage-6 filter row. The row layout is
#: ``idx | bin | rule | w1 | g1 | ig1 | w2 | g2 | ig2`` (9 metadata fields)
#: followed by 4 expected A/B answers and 4 returned A/B answers from the LLM,
#: hence 17 fields total.
_FILTER_RESPONSE_MIN_COLS = 17
...
def _retrieve_correct_visualswap_pairs(
    filter_responses_path: Path,
    final_pairs_path: Path,
) -> int:
    """Collect rows where all four A/B classifications matched the expected answer.

    Reimplements the ``task_type=visualswap`` branch of upstream
    ``retrieve_correct_pairs`` (the OSS release only ships
    ``wordswap`` / ``inflswap`` / ``agrswap``); keeps the upstream code
    in ``third_party/`` untouched.
    """
    accepted = 0
    final_pairs_path.parent.mkdir(parents=True, exist_ok=True)
    seen_pairs: set[str] = set()
    with filter_responses_path.open() as src, final_pairs_path.open("w") as dst:
        for raw in src:
            parts = raw.rstrip().split("|")
            if len(parts) < _FILTER_RESPONSE_MIN_COLS:
                continue
            (
                _idx,
                bin_str,
