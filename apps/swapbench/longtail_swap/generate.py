...
* ``wordswap``:
    ...
    4. LLM filtering -> ``wordswap_sentence_pairs_to_be_filtered.txt``
    5. ``retrieve_correct_pairs --task_type=wordswap`` -> ``wordswap_sentence_pairs_filtered.txt``
...
* ``syntax`` (covers InflectionSwap + AgreementSwap, which share a pipeline):
    ...
    6. LLM filtering -> ``syntax_sentence_pairs_to_be_filtered.txt``
    7. ``retrieve_correct_pairs --task_type=inflswap`` -> ``inflswap_sentence_pairs_filtered.txt``
       and ``--task_type=agrswap`` -> ``agrswap_sentence_pairs_filtered.txt``
...
def _run_upstream(script: str, args: list[str]) -> None:
    ...
    byte-identical — refresh is just a re-copy. PYTHONPATH is extended
...
async def _wordswap(cfg: LTSwapConfig, output_dir: Path) -> None:
    ...
    pairs_to_be_filtered = output_dir / "wordswap_sentence_pairs_to_be_filtered.txt"
    final_pairs = output_dir / "wordswap_sentence_pairs_filtered.txt"
    ...
    _run_upstream(
        "retrieve_correct_pairs",
        [
            f"--input_file={pairs_to_be_filtered}",
            f"--output_file={final_pairs}",
            "--task_type=wordswap",
        ],
    )
...
async def _syntax(cfg: LTSwapConfig, output_dir: Path) -> None:
    ...
    pairs_to_be_filtered = output_dir / "syntax_sentence_pairs_to_be_filtered.txt"
    inflswap_filtered = output_dir / "inflswap_sentence_pairs_filtered.txt"
    agrswap_filtered = output_dir / "agrswap_sentence_pairs_filtered.txt"
    ...
    await run_llm_pool(
        ...
        output_path=inflpairs_filtered,
        temperature=0.0,
        max_tokens=cfg.max_tokens,
        max_retries=cfg.max_retries,
        num_workers=cfg.num_workers,
    ...
    _run_upstream(
        ...
        [
            f"--input_file={pairs_to_be_filtered}",
            f"--output_file={inflswap_filtered}",
            "--task_type=inflswap",
        ],
    )
    _run_upstream(
        ...
        [
            f"--input_file={pairs_to_be_filtered}",
            f"--output_file={agrswap_filtered}",
            "--task_type=agrswap",
        ],
    )
    ...
    _run_upstream(
        ...
        [str(inflswap_filtered), str(inflswap_final), str(agrswap_final)],
    )
