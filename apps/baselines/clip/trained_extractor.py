...
Loads a self-contained ``.pt`` produced by the contrastive trainer (or
emitted by ``egobabyvlm-convert-legacy-ckpt``), instantiates the
matching :class:`MultiModalModel` from the embedded config, and exposes
the standard :class:`core.protocols.MultiModalFeatureExtractor` API used
...
class ContrastiveFeatureExtractor(torch.nn.Module):
    ...
    Args:
        checkpoint_path: Path to a ``.pt`` produced by the contrastive trainer
            or by ``egobabyvlm-convert-legacy-ckpt``. Must have ``model_state_dict``
            and ``config.model`` keys.
