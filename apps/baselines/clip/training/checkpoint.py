...
verbatim, so a feature extractor reads ``torch.load(...)["config"]`` and
...
def save_checkpoint(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "mlm_head_state_dict": _unwrap(mlm_head).state_dict() if mlm_head is not None else None,
...
def load_checkpoint(
    ...
    *,
    ...
    if mlm_head is not None and payload.get("mlm_head_state_dict") is not None:
        _unwrap(mlm_head).load_state_dict(payload["mlm_head_state_dict"])
