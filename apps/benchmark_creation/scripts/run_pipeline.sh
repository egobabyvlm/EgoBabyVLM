...
# Vocabulary extraction runs automatically from the bundled corpus manifests
# in benchmark_creation/resources/.
#
...
#   sbatch benchmark_creation/scripts/run_pipeline.sh \
...
# the benchmark_creation/ package root.  Under SLURM BASH_SOURCE points to
...
SCRIPT_DIR="${_REPO_ROOT}/benchmark_creation/scripts"
...
if [[ -z "$VOCAB_CSV" ]]; then
    ...
    VOCAB_LOG=$(mktemp)
    python -m benchmark_creation.pipeline.vocab_coverage \
        --dataset "$DATASET" \
...
python -m benchmark_creation.pipeline.lexical.build_nouns \
    --vocab-dir "$DATA_DIR" --name "$NAME" \
    --max-per-category "$MAX_NOUNS_PER_CATEGORY" \
    "${LLM_ARGS[@]}" &
PID_NOUNS=$!; track_pid "$PID_NOUNS"

python -m benchmark_creation.pipeline.lexical.build_adjectives \
    --vocab-dir "$DATA_DIR" --name "$NAME" \
...
PID_GRAM=""
if [[ -n "$MODEL" ]]; then
    python -m benchmark_creation.pipeline.grammatical.build_benchmark \
        --vocab-dir "$DATA_DIR" --name "$NAME" \
...
if [[ "$SKIP_IMAGES" != "1" ]]; then
    ...
    PID_GRAM_IMG=""
    if [[ -n "$MODEL" ]]; then
        python -m benchmark_creation.pipeline.grammatical.generate_images \
            --data-dir "$DATA_DIR" --model-id "$IMG_MODEL" \
            --num-gpus "$NUM_GPUS" "${STYLE_FLAGS[@]}" "${IMG_DEBUG_FLAG[@]}" &
...
python -m benchmark_creation.pipeline.manifests.generate_lexical \
