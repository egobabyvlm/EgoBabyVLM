...
def _import_meta_pe() -> tuple[Any, Any]:
    """Import Meta's ``core.vision_encoder`` modules, working around a local ``core`` package shadow."""
    import importlib
    import sys
    from pathlib import Path

    # Fast path: a regular import works (no shadowing).
    try:
        pe_mod = importlib.import_module("core.vision_encoder.pe")
        transforms_mod = importlib.import_module("core.vision_encoder.transforms")
    except ModuleNotFoundError:
        pass
    else:
        return pe_mod, transforms_mod

    # A local 'core' package shadows the installed one. Locate Meta's
    # core/vision_encoder/ in any site-packages on sys.path and prepend
    # that root so the installed package wins.
    for entry in list(sys.path):
        if not entry:
            continue
        candidate = Path(entry) / "core" / "vision_encoder" / "pe.py"
        if not candidate.is_file():
            continue
        # Drop any cached shadowing 'core*' modules and re-import with the
        # correct entry promoted to the front of sys.path.
        ...
        try:
            ...
            # Best-effort cleanup so we don't permanently re-order sys.path
            # for the rest of the process.
            try:
                sys.path.remove(entry)
                # Re-append (preserve original presence if it was there).
    ...
    raise ImportError(
        "Could not locate Meta's perception_models 'core.vision_encoder' package. "
        "Install it via: pip install --no-deps "
